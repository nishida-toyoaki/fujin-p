...
# ── 認可（deny by default） ──
# 現在は admin だけが使える．将来グループへ広げるときは ALLOWED_GROUPS に
# グループコードを並べ，_may_use() にその判定を1つ足せばよい（各ルートは触らない）．
ALLOWED_GROUPS = ()
...
def _gate():
    """入口で一括して閉じる．未ログインは各ルートの admin_required に任せる．"""
...
def play(pres_id):
    """再生専用画面"""
    return render_template('kataribe/play.html', pres_id=pres_id)
